 ...
 * 知乎搜索提供者实现
 */
class ZhihuSearchProvider : SearchProvider {

    override val platform = SearchPlatform.ZHIHU

    companion object {
        private const val TAG = "ZhihuSearchProvider"
    }

    override suspend fun search(context: Context, query: String): SearchResult {
        return try {
            Log.d(TAG, "Searching in Zhihu: $query")
    ...
    override suspend fun openSearchPage(context: Context): SearchResult {
        return try {
            Log.d(TAG, "Opening Zhihu search page")
            ...
            val searchPageUrls = ZhihuSchemes.buildSearchPageUrls()
    ...
    override suspend fun openMainPage(context: Context): SearchResult {
        return try {
            Log.d(TAG, "Opening Zhihu main page")
            ...
            val mainPageUrls = ZhihuSchemes.buildMainPageUrls()
    ...
    override suspend fun openWebVersion(context: Context, query: String): SearchResult {
        return try {
            val webUrl = if (query.isNotEmpty()) {
                ZhihuSchemes.buildWebSearchUrl(query)
            } else {
                ZhihuSchemes.Web.BASE_URL
            }
